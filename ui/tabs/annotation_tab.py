...
class AnnotationTab(BaseTab):  # 继承自 BaseTab
    ...
    def _create_widgets(self):
        ...
        ctk.CTkLabel(parent_frame, text=_("功能注释与富集分析"), font=font_title, text_color=safe_text_color).grid(row=0, column=0, pady=(5, 15), padx=10, sticky="n")
        ...
        ctk.CTkLabel(input_card, text=_("输入数据"), font=font_bold, text_color=safe_text_color).grid(row=0, column=0, columnspan=2, sticky="w", padx=10, pady=(10, 15))
        ctk.CTkLabel(input_card, text=_("基因组版本:"), font=font_regular, text_color=safe_text_color).grid(row=1, column=0, sticky="w", padx=(15, 5), pady=10)
        self.selected_annotation_assembly = tk.StringVar()
        self.assembly_dropdown = ctk.CTkOptionMenu(input_card, variable=self.selected_annotation_assembly, values=[_("加载中...")], font=font_regular, dropdown_font=font_regular)
        self.assembly_dropdown.grid(row=1, column=1, sticky="ew", padx=10, pady=10)
        ctk.CTkLabel(input_card, text=_("基因ID列表:"), font=font_regular, text_color=safe_text_color).grid(row=2, column=0, sticky="nw", padx=(15, 5), pady=10)
        self.annotation_genes_textbox = ctk.CTkTextbox(input_card, height=180, font=font_mono, wrap="word")
        ...
        self.annotation_genes_textbox.bind("<FocusIn>", lambda e: self.app.ui_manager._clear_placeholder(self.annotation_genes_textbox, "genes_input"))
        self.annotation_genes_textbox.bind("<FocusOut>", lambda e: self.app.ui_manager._add_placeholder(self.annotation_genes_textbox, "genes_input"))
        self.annotation_genes_textbox.bind("<KeyRelease>", self._on_gene_input_change)

        anno_card = ctk.CTkFrame(parent_frame, fg_color="transparent")
        anno_card.grid(row=2, column=0, sticky="ew", padx=5, pady=10)
        anno_card.grid_columnconfigure(0, weight=1)
        ctk.CTkLabel(anno_card, text=_("功能注释"), font=font_bold, text_color=safe_text_color).grid(row=0, column=0, sticky="w", padx=10, pady=(10, 15))
        ...
        ctk.CTkCheckBox(checkbox_frame, text="GO", variable=self.go_anno_var, font=font_regular, text_color=safe_text_color).pack(side="left", padx=5)
        ctk.CTkCheckBox(checkbox_frame, text="InterPro", variable=self.ipr_anno_var, font=font_regular, text_color=safe_text_color).pack(side="left", padx=5)
        ctk.CTkCheckBox(checkbox_frame, text="KEGG Orthologs", variable=self.kegg_ortho_anno_var, font=font_regular, text_color=safe_text_color).pack(side="left", padx=5)
        ctk.CTkCheckBox(checkbox_frame, text="KEGG Pathways", variable=self.kegg_path_anno_var, font=font_regular, text_color=safe_text_color).pack(side="left", padx=5)
        self.annotation_output_csv_entry = ctk.CTkEntry(anno_card, placeholder_text=_("输出注释结果路径 (可选, .csv/.xlsx)"), font=font_regular)
        self.annotation_output_csv_entry.grid(row=2, column=0, sticky="ew", padx=10, pady=5)
        self.start_annotation_button = ctk.CTkButton(anno_card, text=_("开始功能注释"), command=self.start_annotation_task, font=font_regular)
        self.start_annotation_button.grid(row=3, column=0, sticky="ew", padx=10, pady=(5, 15))

        enrich_card = ctk.CTkFrame(parent_frame, fg_color="transparent")
        ...
        ctk.CTkLabel(enrich_card, text=_("富集分析与绘图"), font=font_bold, text_color=safe_text_color).grid(row=0, column=0, columnspan=4, sticky="w", padx=10, pady=(10, 15))
        ctk.CTkLabel(enrich_card, text=_("输入格式:"), font=font_regular, text_color=safe_text_color).grid(row=1, column=0, padx=15, pady=5, sticky="w")
        ...
        ctk.CTkCheckBox(input_format_frame, text=_("包含表头"), variable=self.has_header_var, font=font_regular, text_color=safe_text_color).pack(side="left", padx=(0, 15))
        ctk.CTkCheckBox(input_format_frame, text=_("包含Log2FC"), variable=self.has_log2fc_var, font=font_regular, text_color=safe_text_color).pack(side="left")
        ctk.CTkLabel(enrich_card, text=_("分析类型:"), font=font_regular, text_color=safe_text_color).grid(row=2, column=0, padx=15, pady=5, sticky="w")
        self.analysis_type_var = tk.StringVar(value="GO")
        ctk.CTkSegmentedButton(enrich_card, values=["GO", "KEGG"], variable=self.analysis_type_var, font=font_regular).grid(row=2, column=1, padx=10, pady=5, sticky="w")
        ctk.CTkLabel(enrich_card, text=_("绘图类型:"), font=font_regular, text_color=safe_text_color).grid(row=3, column=0, padx=15, pady=5, sticky="w")
        ...
        ctk.CTkCheckBox(plot_type_frame, text=_("气泡图"), variable=self.bubble_plot_var, font=font_regular, text_color=safe_text_color).pack(side="left", padx=(0, 15))
        ctk.CTkCheckBox(plot_type_frame, text=_("条形图"), variable=self.bar_plot_var, font=font_regular, text_color=safe_text_color).pack(side="left", padx=(0, 15))
        ctk.CTkCheckBox(plot_type_frame, text=_("Upset图"), variable=self.upset_plot_var, font=font_regular, text_color=safe_text_color).pack(side="left", padx=(0, 15))
        ctk.CTkCheckBox(plot_type_frame, text=_("网络图(Cnet)"), variable=self.cnet_plot_var, font=font_regular, text_color=safe_text_color).pack(side="left")

        ctk.CTkLabel(enrich_card, text=_("输出目录:"), font=font_regular, text_color=safe_text_color).grid(row=4, column=0, padx=15, pady=5, sticky="w")
        self.enrichment_output_dir_entry = ctk.CTkEntry(enrich_card, font=font_regular)
        self.enrichment_output_dir_entry.grid(row=4, column=1, columnspan=2, sticky="ew", padx=(10, 5), pady=5)
        ctk.CTkButton(enrich_card, text=_("浏览..."), width=100, command=lambda: self.app.event_handler._browse_directory(self.enrichment_output_dir_entry), font=font_regular).grid(row=4, column=3, padx=(0, 10), pady=5)
        self.start_enrichment_button = ctk.CTkButton(enrich_card, text=_("开始富集分析"), command=self.start_enrichment_task, font=font_regular)
