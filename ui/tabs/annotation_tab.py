...
class AnnotationTab(BaseTab):
    ...
    def _create_widgets(self):
        ...
        ttk.Label(parent_frame, text=_("功能注释"), font=self.app.app_title_font, bootstyle="primary").grid(
        ...
        anno_card = ttkb.LabelFrame(parent_frame, text=_("注释类型"), bootstyle="secondary")
        ...
        ttkb.Button(anno_card, text=_("浏览..."), width=12,
                    command=lambda: self.app.event_handler._browse_save_file(
                        self.annotation_output_csv_entry,
                        [("CSV files", "*.csv"), ("All files", "*.*")]
                    ),
                    bootstyle="info-outline").grid(row=1, column=2, padx=(0, 10), pady=5)  # 新增一列放置按钮
    ...
    def update_from_config(self):
        ...
        # 设置功能注释的默认输出目录
        default_anno_dir = os.path.join(os.getcwd(), "annotation_results")
    ...
    def update_button_state(self, is_running, has_config):
        ...
        # 移除了对 start_enrichment_button 的更新
    ...
    def start_annotation_task(self):
        if not self.app.current_config:
            self.app.ui_manager.show_error_message(_("错误"), _("请先加载配置文件。")); return
        ...
        if not output_path: # 如果用户未指定，则使用默认输出路径
            output_path = os.path.join(os.getcwd(), "annotation_results", f"annotation_result_{assembly_id}.csv")

        if not gene_ids:
            self.app.ui_manager.show_error_message(_("输入缺失"), _("请输入要注释的基因ID。")); return
        if not assembly_id or assembly_id in [_("加载中..."),
                                              _("无可用基因组")]:
            self.app.ui_manager.show_error_message(_("输入缺失"), _("请选择一个基因组版本。")); return
        if not anno_types:
            self.app.ui_manager.show_error_message(_("输入缺失"), _("请至少选择一种注释类型。")); return
        ...
        task_kwargs = {
            ...
            'bridge_species': assembly_id, # 功能注释不需要桥梁物种，可以直接传入目标基因组ID
            'annotation_types': anno_types,
            'output_path': output_path, # 直接传递计算好的输出路径
            'output_dir': os.path.dirname(output_path) # output_dir 传递 output_path 的目录部分
        }
