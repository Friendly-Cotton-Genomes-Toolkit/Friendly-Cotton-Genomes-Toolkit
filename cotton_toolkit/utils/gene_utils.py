...
from cotton_toolkit.external_functions.sequence_evolution import _, logger
...
def validate_cds_fasta(file_path: str):
    """
    验证一个文件是否为适用于下游密码子分析的、有效的CDS FASTA文件。

    该函数会执行以下检查:
    1. 文件是否存在且非空。
    2. 文件是否为有效的FASTA格式。
    3. 文件中每条序列的长度是否为3的倍数。
    4. 文件中每条序列是否包含内部（非末尾）的终止密码子。

    如果任何检查失败，将引发一个带有详细错误信息的 `ValueError`。

    Args:
        file_path (str): 需要验证的FASTA文件的路径。

    Raises:
        ValueError: 如果文件未通过任何验证检查。
    """
    logger.info(_("正在验证CDS FASTA文件: {}").format(file_path))

    # 检查1: 文件是否存在且非空
    try:
        if os.path.getsize(file_path) == 0:
            raise ValueError(_("输入文件 '{}' 为空。").format(file_path))
    except FileNotFoundError:
        raise ValueError(_("输入文件 '{}' 未找到。").format(file_path))
    except OSError as e:
        raise ValueError(_("无法读取文件 '{}': {}").format(file_path, e))
    ...
    try:
        ...
        for record in SeqIO.parse(file_path, "fasta"):
            is_fasta = True  # 如果能进入循环，说明至少是FASTA格式
            seq = record.seq
            seq_len = len(seq)

            # 检查3: 序列长度是否为3的倍数
            if seq_len % 3 != 0:
                invalid_length_seqs.append(record.id)

            # 检查4: 是否有内部终止密码子
            # 将序列翻译成蛋白质，检查除末尾位置外，是否出现终止符 '*'
            protein = seq.translate()
            if "*" in protein[:-1]:
                internal_stop_seqs.append(record.id)

    except Exception as e:
        # 如果解析失败，说明文件格式有问题
        raise ValueError(_("无法将文件 '{}' 解析为FASTA格式。请检查文件内容。\n错误详情: {}").format(file_path, e))

    if not is_fasta:
        raise ValueError(_("文件 '{}' 不包含任何有效的FASTA序列记录。").format(file_path))

    # 在所有检查完成后，统一报告所有发现的问题
    error_messages = []
    if invalid_length_seqs:
        error_messages.append(
            _("错误：以下序列的长度不是3的倍数，不适用于密码子分析:\n{}").format(
                ", ".join(invalid_length_seqs[:5]) + ("..." if len(invalid_length_seqs) > 5 else "")
            )
        )
    if internal_stop_seqs:
        error_messages.append(
            _("错误：以下序列包含内部终止密码子(*)，请检查其阅读框或序列质量:\n{}").format(
                ", ".join(internal_stop_seqs[:5]) + ("..." if len(internal_stop_seqs) > 5 else "")
            )
        )

    if error_messages:
        # 如果有任何错误，合并所有错误信息并引发一次异常
        raise ValueError("\n\n".join(error_messages))
...
def translate_cds_to_protein(
        input_cds_path: str,
        output_protein_path: str,
        progress_callback: Optional[Callable[[float, str], None]] = None,
        cancel_event: Optional[Event] = None
):
    """将CDS FASTA文件翻译成蛋白质FASTA文件"""
    step_name = _("翻译CDS为蛋白质")
    ...
    for record in SeqIO.parse(input_cds_path, "fasta"):
        ...
        # 确保序列长度是3的倍数，如果不是，则进行截断
        trimmed_len = len(record.seq) - (len(record.seq) % 3)
        if trimmed_len < len(record.seq):
            logger.warning(_("序列 {} 长度不是3的倍数，将截断以进行翻译。").format(record.id))

        protein_seq = record.seq[:trimmed_len].translate(to_stop=True)
        record.seq = protein_seq
        proteins.append(record)

    SeqIO.write(proteins, output_protein_path, "fasta")
    if progress_callback: progress_callback(100, _("翻译完成。"))
